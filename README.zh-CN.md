...
<p align="center"><a href="#以话题为核心的记忆">理解话题</a> · <a href="#实测效果">实测效果</a> · <a href="#软件界面">软件界面</a> · <a href="#项目定位与-opencode">项目定位</a> · <a href="#快速开始">快速开始</a> · <a href="#系统架构">系统架构</a> · <a href="#模型与权重">模型与权重</a> · <a href="#构建与验证">构建与验证</a> · <a href="#文档导航">文档导航</a></p>
...
同一事项会跨越多次会话，一次会话也可能切换到其他事项。只沿着会话时间线查找，容易把同一件事的前因后果分散开，也容易把同一项目里的不同目标混在一起。**话题提供稳定的事项身份和边界，事件仍保留原来的来源与时间。**

![为什么按话题组织：将不同会话里交错出现的交付报告与网络排障事件，分别归入两个持续话题。](docs/assets/evaluation/why-topics-zh.png)

这张图解释组织方式与设计目的；它不代表已经完成“有话题 / 无话题”的性能对照实验。

本轮隔离功能探针已确认：两个明确绑定的会话可以继续同一话题，绑定在服务重启后仍保留。这是特定行为检查，完整探针结果（含失败项）见[评测说明](docs/evaluation/README.zh-CN.md)。
...
2026-09-17，在 Apple M1 Pro / 16 GiB 上重跑当前公开代码：53 个话题、284 条合成事件、246 个唯一问题（234 个正例），两配置各 3 轮。**两组都采用相同的话题结构，下面比较的是加入 TIDE FP32 编码器后的检索变化。**

![实测效果：相同话题结构下，词法/结构化检索与 TIDE FP32 的话题命中、证据召回和耗时对照。](docs/assets/evaluation/retrieval-tradeoff-zh.png)

- 话题 Top-1：**65.38% → 82.48%**，提高 17.09 个百分点。
- 证据 Recall@10（宏平均）：**81.27% → 92.59%**；找齐全部证据从 **187/234** 增至 **214/234**。
- 本机检索 P95：**10.83 → 83.22 ms**。召回改善伴随推理开销，不含启动、Electron 和大模型生成耗时。

在 48 个自编正例的桌面服务上下文检查中，未绑定条件下完整证据覆盖为 **44/48**；评测器直接提供正确话题 ID 时为 **46/48**。后者不是模型自动归题成绩，且有请求顺序和缓存影响。

<details>
<summary>查看全部类别：哪些改善，哪些仍是弱项</summary>

![全部正例类别的 Top-1 对照与样本数。](docs/assets/evaluation/category-results-zh.png)

相似事项混淆组 FP32 仅 **8/16（50%）**，临时选择/作用域组为 **3/7（42.86%）**，均低于本轮词法配置。小样本类别的高分不代表真实场景可靠性。

</details>

这套语料包含开发题，未完成独立人工盲评，训练重合未核验；未测最终大模型回答准确率或麒麟目标机。专项边界检查仍有未通过项，详见[完整评测说明、失败范围和复现命令](docs/evaluation/README.zh-CN.md)。[指标 JSON](docs/evaluation/2026-09-17/measurements.json)与精简请求明细已公开，图表可由源码重新生成。
