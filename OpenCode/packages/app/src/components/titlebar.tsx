...
import { MemPulseMark } from "@/memory/components/brand"
...
function TitlebarV2Right(props: { state: TitlebarV2RightState }) {
  ...
  const language = useLanguage()
  const command = useCommand()
  const memoryKeybind = createMemo(() => command.keybindParts("memory.open"))

  return (
    <div class="relative z-20 flex shrink-0 items-center justify-end gap-1.5 overflow-visible mr-2">
      <TooltipV2
        placement="bottom"
        value={
          <>
            {language.t("memory.title")}
            <Show when={memoryKeybind().length > 0}>
              <KeybindV2 keys={memoryKeybind()} variant="neutral" />
            </Show>
          </>
        }
      >
        <IconButtonV2
          size="small"
          variant="ghost"
          aria-label={language.t("memory.title")}
          onClick={() => navigate("/")}
          icon={<MemPulseMark size={16} />}
        />
      </TooltipV2>
