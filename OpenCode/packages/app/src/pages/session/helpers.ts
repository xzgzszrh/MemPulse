...
export const createSessionTabs = (input: TabsInput) => {
  ...
  // Memory is a fixed panel like context: always available, never persisted as
  // a closable tab, so it does not take part in the drag-reorder list.
  const memoryOpen = createMemo(() => input.tabs().active() === "memory")
  ...
  const panelTabs = createMemo(
    () => {
      ...
      return input
        ...
        .all()
        .flatMap((tab) => {
          if (tab === "context" || tab === "review" || tab === "memory") return []
