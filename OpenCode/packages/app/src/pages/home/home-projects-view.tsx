...
import { MEMORY_SECTIONS, type HomeSection } from "@/memory/home/sections"
import { sectionIcon, sectionLabelKey } from "@/memory/home/section-labels"
...
export function HomeProjectsView(props: HomeProjectsViewProps) {
  ...
  // The sidebar always lives in the fixed left column of the home shell, so it
  // fills that column regardless of whether sessions or a memory section is shown.
  return (
    <aside
      class="flex h-full min-h-0 min-w-0 flex-col gap-4 overflow-hidden pt-3 pb-4"
    ...
    >
      ...
      <div class="flex shrink-0 flex-col gap-1 pr-3">
        <div class="mx-1.5 h-px bg-v2-border-border-base" />
        <div class="px-1.5 pt-2 text-v2-text-text-muted [font-weight:530]">{props.language.t("memory.title")}</div>
        <For each={MEMORY_SECTIONS}>
          {(item) => (
            <HomeProjectNavButton
              type="button"
              data-selected={props.section() === item ? "" : undefined}
              class="text-v2-text-text-faint [&>[data-slot=icon-svg]]:text-v2-icon-icon-muted"
              onClick={() => props.onSelectSection(item)}
            >
              <Icon name={sectionIcon(item)} size="small" />
              <span class={HOME_PROJECT_NAV_LABEL}>{props.language.t(sectionLabelKey(item))}</span>
            </HomeProjectNavButton>
          )}
        </For>
      </div>
