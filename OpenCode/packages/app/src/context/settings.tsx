...
export interface Settings {
  ...
  /** Presentation-only memory preferences. Capture and retrieval policy live in the MemPulse service. */
  memory: MemorySettings
}

export interface MemorySettings {
  /** Show the memory panel as a tab in the session side panel. */
  showSessionTab: boolean
  /** Poll the local service while a memory surface is open. */
  liveRefresh: boolean
}
...
export const { use: useSettings, provider: SettingsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      memory: {
        showSessionTab: withFallback(() => store.memory?.showSessionTab, defaultSettings.memory.showSessionTab),
        setShowSessionTab(value: boolean) {
          setStore("memory", "showSessionTab", value)
        },
        liveRefresh: withFallback(() => store.memory?.liveRefresh, defaultSettings.memory.liveRefresh),
        setLiveRefresh(value: boolean) {
          setStore("memory", "liveRefresh", value)
        },
      },
    }
  },
