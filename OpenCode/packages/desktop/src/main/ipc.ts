...
type Deps = {
  ...
  mempulseRequest: (request: { method: string; params?: Record<string, unknown> }) => Promise<unknown>
}
...
export function registerIpcHandlers(deps: Deps) {
  ...
  ipcMain.handle("mempulse-request", (event: IpcMainInvokeEvent, request: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    if (!win || win.isDestroyed() || win.webContents !== event.sender || event.senderFrame !== event.sender.mainFrame) {
      throw new Error("Invalid memory request sender")
    }
    if (!request || typeof request !== "object" || typeof (request as { method?: unknown }).method !== "string") {
      throw new Error("Invalid memory request")
    }
    const value = request as { method: string; params?: Record<string, unknown> }
    return deps.mempulseRequest(value)
  })
